...
class GroupMembersCog(commands.Cog):
    ...
    async def count(self, inter: AppCmdInter, item_type: Literal["person", "group"],
        ...
        await inter.send(f"There is **{len(medias)}** known media for that {item_type}.")
    ...
    async def aliases(self, inter: AppCmdInter, name):
        ...
        embed = disnake.Embed(title=f"Aliases for {name}", color=helper.get_random_color(), description=description)
    ...
    async def distance(self, inter: AppCmdInter, search_word: str, target_word: str):
        ...
        await inter.send(f"The search word has **{helper.levenshtein_distance(search_word, target_word) * 100:.2f}%** "
    ...
    async def countleaderboard(self, inter: AppCmdInter, item_type: Literal["Persons", "Groups"]):
        ...
        embed = disnake.Embed(title=f"{item_type} Leaderboard", color=helper.get_random_color(), description=desc)
        await inter.send(embed=embed)
