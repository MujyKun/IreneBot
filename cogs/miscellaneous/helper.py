...
async def process_server_info(guild: disnake.Guild, inter=None, ctx=None):
    ...
    embed = await botembed.create_bot_author_embed(
        title=f"{guild.name} ({guild.id})", url=f"{guild.icon.url}"
    )
...
async def process_random(
    start_number, end_number, invoker_user_id, ctx=None, inter=None
):
    """Process a random number."""
    user = await User.get(invoker_user_id)
    if start_number > end_number:
        return await send_message(
            key="invalid_range", user=user, ctx=ctx, inter=inter, ephemeral=True
        )
    return await send_message(
        start_number,
        end_number,
