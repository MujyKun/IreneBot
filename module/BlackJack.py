...
class BlackJack(commands.Cog):
        ...
        try:
            ...
            if versus != "bot":
                if await self.ex.u_blackjack.process_bj_game(ctx, amount, user_id):
                    await self.ex.u_blackjack.add_bj_game(user_id, amount, ctx, "player")
            else:
                if await self.ex.u_blackjack.process_bj_game(ctx, amount, user_id):
    ...
    async def joingame(self, ctx, game_id=0, amount="0"):
        ...
        try:
            ...
            user_id = ctx.author.id
            if await self.ex.u_blackjack.process_bj_game(ctx, amount, user_id):
                game = await self.ex.u_blackjack.get_game(game_id)
                if game is None:
    ...
    async def endgame(self, ctx):
        ...
        try:
            ...
            if game_id is None:
                ...
                await self.ex.u_blackjack.delete_game(game_id)
    ...
    async def hit(self, ctx):
        ...
        try:
            ...
            if not game_id:
                ...
                if await self.ex.u_blackjack.compare_channels(ctx.author.id, ctx.channel):
                    game = await self.ex.u_blackjack.get_game(game_id)
                    if self.ex.u_blackjack.check_if_bot(game[2]):
                        if await self.ex.u_blackjack.get_player_total(game[2]) < 16:
                            await self.ex.u_blackjack.add_card(game[2])
                    await self.ex.u_blackjack.add_card(ctx.author.id)
        except Exception as e:
            log.console(e)
    ...
    async def stand(self, ctx):
        ...
        try:
            ...
            if game_id is None:
                ...
                if await self.ex.u_blackjack.compare_channels(user_id, ctx.channel):
                    ...
                    await self.ex.u_blackjack.set_player_stand(user_id)
                    game = await self.ex.u_blackjack.get_game(game_id)

                    if self.ex.u_blackjack.check_if_bot(game[2]):
                        check = True
                        await self.ex.u_blackjack.finish_game(game_id, ctx.channel)
