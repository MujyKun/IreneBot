...
client = keys.client
patron_message = f"""
**Non Patrons can only send 150 photos a day maximum. 
...
commands_used = {"reset_time": time.time()}
"""
{
    reset_time: date
...
def check_reset_limits():
    ...
    if time.time() - commands_used['reset_time'] > 86400:  # 1 day in seconds
        # reset the dict
        commands_used = {"reset_time": time.time()}
...
def add_user_limit(message_sender):
    global commands_used
    if message_sender.id not in commands_used:
        commands_used[message_sender.id] = [1, time.time()]
    else:
        commands_used[message_sender.id] = [commands_used[message_sender.id][0] + 1, time.time()]


async def check_user_limit(message_sender, message_channel):
    if message_sender.id in commands_used:
        if not await ex.check_if_patreon(message_sender.id) and commands_used[message_sender.id][0] > keys.idol_post_send_limit:
                ...
                if not await ex.check_if_temp_channel(message_channel.id):
                    if await ex.check_channel_sending_photos(message_channel.id):
                        ...
                        try:
                            check_reset_limits()
                            if message_sender.id in commands_used:
                                time_difference = time.time() - commands_used[message_sender.id][1]
