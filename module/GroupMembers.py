...
class GroupMembers(commands.Cog):
    ...
    async def on_message2(message):
        ...
        if not message.author.bot and await ex.u_group_members.check_channel_sending_photos(channel.id) and not await ex.check_if_temp_channel(channel.id):
            try:
    ...
    async def randomidol(self, ctx):
        ...
        if await ex.u_group_members.check_channel_sending_photos(ctx.channel.id) and not await ex.check_if_temp_channel(ctx.channel.id):
            channel = ctx.channel
            try:
