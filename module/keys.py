...
def env_to_list(key):
    try:
        ...
        log.useless(f"{e} (Exception) -> Was not able to separate list.", method=env_to_list)
        return []
...
def get_and_reload_keys() -> Keys_Obj:
    """
    Will get the Keys object after reloading the env variables.

    :returns: IreneUtility.models.keys.Keys
    """
    ...
    lyric_client = None if not len(lyrics_api_key) else ksoftapi.Client(lyrics_api_key)
    ...
    key_kwargs = {
        ...
        "twitter_update_limit": make_int(os.getenv("TWITTER_UPDATE_LIMIT")),
        ...
        # use the first result in the list of keys.
    ...
    return Keys_Obj(**key_kwargs)


keys_obj = get_and_reload_keys()
