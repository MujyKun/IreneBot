...
class BiasGame:
    async def create_bias_game_image(self, first_idol_id, second_idol_id):
        ...
        return f"{ ex.keys.bias_game_location}{first_idol_id}_{second_idol_id}.png"
    ...
    def merge_images(first_idol_id, second_idol_id):
        ...
        if not ex.check_file_exists(f"{ex.keys.bias_game_location}{file_name}"):
            # open the images.
            with Image.open(f'{ex.keys.bias_game_location}versus.png') as versus_image, \
                    Image.open(f'{ex.keys.idol_avatar_location}{first_idol_id}_IDOL.png') as first_idol_image, \
                    Image.open(f'{ex.keys.idol_avatar_location}{second_idol_id}_IDOL.png') as second_idol_image:
                ...
                versus_image.save(f"{ex.keys.bias_game_location}{file_name}")
    ...
    async def create_bias_game_bracket(self, all_games, user_id, bracket_winner):
        ...
        return f"{ex.keys.bias_game_location}{user_id}.png"
    ...
    def create_bracket(all_games, user_id, bracket_winner):
        ...
        with Image.open(f'{ex.keys.bias_game_location}bracket8.png') as bracket:
            count = 1
            for c_round in all_games:
                ...
                for first_idol, second_idol in c_round:
                    ...
                    with Image.open(f'{ex.keys.idol_avatar_location}{first_idol.id}_IDOL.png') as first_idol_image, \
                            Image.open(f'{ex.keys.idol_avatar_location}{second_idol.id}_IDOL.png') as second_idol_image:
            ...
            with Image.open(f'{ex.keys.idol_avatar_location}{bracket_winner.id}_IDOL.png') as idol_image:
