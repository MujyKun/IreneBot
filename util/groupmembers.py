...
class GroupMembers:
    ...
    async def set_global_alias(self, obj, alias):
        ...
        await self.conn.execute("INSERT INTO groupmembers.aliases(objectid, alias, isgroup) VALUES($1, $2, $3)", obj.id,
                                alias, is_group)
    ...
    async def get_db_idol_called(self, member_id):
        ...
        return self.first_result(
            await self.conn.fetchrow("SELECT Count FROM groupmembers.Count WHERE MemberID = $1", member_id))
    ...
    async def get_db_members_in_group(self, group_name=None, group_id=None):
        ...
        if group_id is None:
            group_id = self.first_result(
                await self.conn.fetchrow(f"SELECT groupid FROM groupmembers.groups WHERE groupname = $1", group_name))
    ...
    async def add_idol_to_group(self, member_id: int, group_id: int):
        return await self.conn.execute("INSERT INTO groupmembers.idoltogroup(idolid, groupid) VALUES($1, $2)",
                                       member_id, group_id)

    async def remove_idol_from_group(self, member_id: int, group_id: int):
        return await self.conn.execute("DELETE FROM groupmembers.idoltogroup WHERE idolid = $1 AND groupid = $2",
                                       member_id, group_id)
    ...
    async def send_names(self, ctx, mode, user_page_number=1, group_ids=None):
        ...
        async def check_mode(embed_temp):
            ...
            if mode == "fullname":
                embed_temp = await self.set_embed_author_and_footer(embed_temp,
    ...
    async def delete_dead_link(self, link, idol_id):
        return await self.conn.execute("DELETE FROM groupmembers.deadlinkfromuser WHERE deadlink = $1 AND idolid = $2",
                                       link, idol_id)

    async def set_forbidden_link(self, link, idol_id):
        return await self.conn.execute("INSERT INTO groupmembers.forbiddenlinks(link, idolid) VALUES($1, $2)", link,
                                       idol_id)
    ...
    async def update_member_count(self, idol):
        ...
        if not idol.called:
            ...
            idol.called += 1
            await self.conn.execute("UPDATE groupmembers.Count SET Count = $1 WHERE MemberID = $2", idol.called,
                                    idol.id)
    ...
    async def get_google_drive_link(self, api_url):
        ...
        return self.first_result(
            await self.conn.fetchrow("SELECT driveurl FROM groupmembers.apiurl WHERE apiurl = $1", str(api_url)))
